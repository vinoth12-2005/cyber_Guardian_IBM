...
const TUTOR_SYSTEM_PROMPT =
    "You are FlotBot, a premier AI cybersecurity assistant.\n" +
    "Your objective is to provide short, clear, and actionable security responses.\n\n" +
    "CORE RESPONSE STYLE RULES:\n" +
    "1. DEFAULT BREVITY: Keep normal responses concise (2 to 5 short lines, max 8 lines). Avoid long paragraphs, filler, or raw API dumps.\n" +
    "2. ACCURACY & GROUNDING: Ground answers strictly in provided evidence. State provider status clearly. Never claim an ERROR/UNAVAILABLE provider gave a verdict.\n" +
    "3. SECURITY ALERT FORMATS (Use when reporting URL or File security results unless user explicitly requests full technical details):\n" +
    "   • Malicious Threat:\n" +
    "     🔴 THREAT DETECTED\n" +
    "     Target: <file/URL>\n" +
    "     Risk: HIGH / CRITICAL\n" +
    "     Why: <1 short sentence summarising active provider match>\n" +
    "     Action: <1 clear recommendation>\n" +
    "   • Safe Result:\n" +
    "     🟢 NO KNOWN THREAT\n" +
    "     Target: <file/URL>\n" +
    "     Why: <1 short sentence on clean provider check>\n" +
    "   • Suspicious Result:\n" +
    "     🟡 SUSPICIOUS\n" +
    "     Target: <file/URL>\n" +
    "     Risk: MEDIUM\n" +
    ...
    "     Action: <1 clear recommendation>\n" +
    "4. MULTI-PROVIDER & ERROR HANDLING SUMMARY:\n" +
    "   - If multiple providers agree: Summarise concisely (e.g., 'VirusTotal and Google Safe Browsing both flagged this URL.').\n" +
    "   - If a provider fails: Explicitly state status (e.g., 'VirusTotal: Malicious | Google Safe Browsing: Unavailable | Overall: Malicious based on VirusTotal evidence.').\n" +
    "5. DETAILED MODE: Provide full technical evidence, YARA matches, or deep breakdowns ONLY if the user explicitly asks for 'details', 'technical details', 'evidence', 'provider results', 'why', 'full report', or 'explain in detail'.";
...
class AIEngine {
    ...
    _buildChatMessages(message, history, context) {
        ...
        // ─── 1. Check for Authoritative URL Security Intelligence (DetectionManager) ───────────────
        if (context.urlSecurityReport) {
            const reports = Array.isArray(context.urlSecurityReport) ? context.urlSecurityReport : [context.urlSecurityReport];
            let reportBlocks = "";
            for (const r of reports) {
                const providersStr = r.provider_results
                    ? r.provider_results.map(p => {
                        if (p.status === "completed") {
                            return `[${p.provider}] STATUS: COMPLETED | VERDICT: ${p.classification.toUpperCase()} | Evidence: ${JSON.stringify(p.evidence || {})}`;
                        } else if (p.status === "error") {
                            return `[${p.provider}] STATUS: ERROR (REQUEST FAILED) | VERDICT: NO VERDICT / ERROR | Details: ${p.error || "Request failed"}`;
                        } else {
                            return `[${p.provider}] STATUS: UNAVAILABLE | VERDICT: NO VERDICT / UNAVAILABLE | Details: ${p.error || "Unconfigured"}`;
                        }
                    }).join("\n  ")
                    : "None";
                ...
                reportBlocks +=
                    `\n- Target URL: ${r.url}` +
                    `\n- Fused Authoritative Classification: ${r.classification}` +
                    `\n- Risk Level: ${r.riskLevel} (Confidence: ${r.confidence}%)` +
                    `\n- Provider Breakdown:\n  ${providersStr}` +
                    `\n- Local Telemetry Warnings: ${r.warnings ? r.warnings.join("; ") : "None"}` +
                    `\n- Summary: ${r.summary}`;
            }

            contextAddition +=
                `\n\n[AUTHORITATIVE EXTERNAL SECURITY INTELLIGENCE (VirusTotal & Google Safe Browsing):` +
                reportBlocks +
                `\n- CRITICAL ACCURACY RULES FOR LLM EXPLANATION:\n` +
                `  1. Use the concise 2-5 line format (🔴 THREAT DETECTED / 🟢 NO KNOWN THREAT / 🟡 SUSPICIOUS) unless the user explicitly requested full technical details/evidence.\n` +
                `  2. State the exact verdict for each provider based ONLY on its STATUS.\n` +
                `  3. If a provider's status is ERROR or UNAVAILABLE, state explicitly that it returned no verdict due to service failure/missing key.\n` +
                `  4. NEVER claim both or all providers flagged a threat if any provider had an ERROR or UNAVAILABLE status!\n` +
                `  5. Ground your answer strictly in the provider breakdown provided above.]`;
        }
        else if (discussedUrl && (urlMatch || /\b(url|site|page|website|link|click|suspicious|safe|benign|mcafee|domain)\b/i.test(lowerMsg))) {
