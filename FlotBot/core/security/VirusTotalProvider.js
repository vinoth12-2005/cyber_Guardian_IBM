    ...
    async checkUrl(urlString) {
        ...
        try {
            ...
            return new SecurityResult({
                ...
                status: "completed",
                classification,
                confidence: total > 0 ? Math.min(1.0, (malicious + suspicious) / Math.max(1, total)) : 0.0,
                detections: { malicious, suspicious, total },
                evidence: { stats }
            });
        } catch (err) {
            if (err.response?.status === 404) {
                return new SecurityResult({
                    ...
                    status: "completed",
                    classification: "unknown",
                    evidence: { details: "URL not previously scanned in VirusTotal database" }
                });
            }
            return SecurityResult.error("VirusTotal", "url", urlString, err.message || "API error");
        }
    }
}
