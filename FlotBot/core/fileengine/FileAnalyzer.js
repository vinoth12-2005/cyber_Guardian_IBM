...
class FileAnalyzer {
    ...
    async analyzeFile(filePath, contextOptions = {}) {
        ...
        // 15. Outcome Classification (External Intelligence is primary authority, local heuristics provide evidence)
        let verdict = "CLEAN";
        const hasExternalMalicious = (threatIntel && (threatIntel.verdict === "MALICIOUS" || threatIntel.maliciousCount >= 2));
        const hasExternalSuspicious = (threatIntel && (threatIntel.verdict === "SUSPICIOUS" || threatIntel.maliciousCount > 0));

        if (reputation.verdict === "KNOWN_BENIGN") {
            verdict = "KNOWN_BENIGN";
        } else if (reputation.verdict === "KNOWN_MALICIOUS" || hasExternalMalicious) {
            verdict = "MALICIOUS";
        } else if (hasExternalSuspicious || riskResult.riskScore >= 65) {
            verdict = "SUSPICIOUS";
        } else if (riskResult.riskScore >= 25) {
            verdict = "UNUSUAL";
        } else if (!threatIntel || threatIntel.verdict === "UNKNOWN") {
