/**
 * URLEngine - Standalone Threat Detection Module
 * ─────────────────────────────────────────────────────────────
...
class URLEngine {
  ...
  static analyze(rawUrl) {
    ...
    const isAnyOfficialBrand = URLEngine.BRAND_TARGETS.some((t) =>
    ...
    if (!isAnyOfficialBrand && matchedKeywords.length > 0) {
      if (matchedKeywords.length >= 2 || /(-|_)/.test(hostname)) {
        ...
        score += 40;
        mitre.push('T1566.002');
      } else if (protocol === 'http:' || URLEngine.SUSPICIOUS_TLDS.has(tld)) {
        warnings.push(
    ...
    else if (finalScore >= 15) riskLevel = 'LOW';
